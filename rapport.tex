...
\begin{proposition}
    ...
    Soit $\pi \in Sym(S_n^m)$, on a
    \[ [a] \in \mathfrak{J} \implies \pi([a]) \in \mathfrak{J} \]
\end{proposition}

\begin{proof}
    Il suffit d'observer que $E_a \cong E_{\pi([a])}$
\end{proof}
...
\begin{proposition}
    ...
    \begin{align*}
        {([a_1], \dots, [a_m])}_n \in \mathfrak{J} \implies {([a_1], \dots, [a_{m-1}])}_n \in \mathfrak{J}
    \end{align*}
\end{proposition}

\begin{proof}
    Notons $[a] = {([a_1], \dots, [a_m])}_n$
    
    Supposons $[a] \in \mathfrak{J}$, donc $\forall [b] \in E_a : [b] \in \mathfrak{J}$

    La preuve se fait par induction structurelle

    D'abord, on observe ${([1], \dots, [1])}_n \in \mathfrak{J}$

    Puis on suppose que $\forall [b] \in E_a : [b] \in \mathfrak{J} \implies {([b_1], \dots, [b_{m-1}])}_n \in \mathfrak{J}$

    On en conclut ${([a_1], \dots, [a_{m-1}])}_n \in \mathfrak{J}$

\end{proof}

Illustration pour $m = 3$

\begin{figure}[h]
    \caption{${[(a, b, c)]}_n \twoheadrightarrow{} {[(a, b)]}_n$}
    \centering
    \includegraphics[width=0.3\textwidth]{abcn}
    \includegraphics[width=0.3\textwidth]{abn}
\end{figure}
...
%     Donc, sans perdre de généralité, $\exists k \in \{3,\dots,m\} : a_1 > \cdots > a_k > 1$

%     On a donc les singularités $[a^1], \dots, [a^k] \in E_a$ d'éclatements naturels $a^1, \dots, a^k$

%     Si $\exists i \in \{1,\dots,k\} : |E_{a^i}| > 2$, on répète le raisonnement avec (par ($\star$))

%     Supposons donc $\forall i \in \{1,\dots,k\} : |E_{a^i}| \leq 2$

%     Considérons $a^1 = (a^1_1, \dots, a^1_m)$

%     Puisque $a_1 > a_2 > a_3 > 1$, on a $a^1_2 > a^1_3 > 1$

%     Forcément $|E_{a^1}| = 2$ et $(-n \mod {a_1}) = 1$
